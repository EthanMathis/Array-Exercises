...
export const getStudentsInCohort = (cohort) => {
    return students.filter(x => x.cohort === cohort)
}
...
export const getAvailableInstructors = () => {
    let teacher = students.map(kid => kid.instructorId)
    return instructors.filter(instOBJ =>{
        if (teacher.includes(instOBJ.id) === false) {
            return instOBJ
        }
    })
}
...
export const getStudentsByLanguage = (language) => {
    return students.filter(kid => kid.languages.some(lang => lang === language))
}
